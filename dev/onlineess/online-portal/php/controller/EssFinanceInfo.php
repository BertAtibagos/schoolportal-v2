<?php 
	...
	if ($_GET['type'] == 'GET_NO_OF_STUDENTS_WITH_PAYMENT') {
			...
			$qry = "SELECT 	COUNT(*) `COUNT` 
					...
					WHERE 	
						...
						WHERE 	(CASE IFNULL(`schl_enr_reg`.`SchlEnrollRegSms_ID`, 0)
								...
								END) = `oc_enr_pay`.`registration_id` AND 
		...
		for ($x=0; $x < $counter; $x++) { 
			...
			foreach ($cont_open_schl_acad_yrprd as $value) {
				...
				$qry = "SELECT (CASE IFNULL(`schl_enr_reg`.`SchlEnrollRegSms_ID`, 0)
							...
							UPPER(CONCAT(	`oc_enr_pay`.`last_name`, ', ', 
											`oc_enr_pay`.`first_name` , ' ', 
											`oc_enr_pay`.`middle_name`)) `NAME`,
							...
							`schl_acad_prd`.`SchlAcadPrd_NAME` `PRD_NAME`,
							...
							IFNULL(`schl_acad_asmt`.`SchlEnrollAss_STATUS`, 0) `ASMT_STATUS`,
						...
						FROM	`oc_enrollment_payments` `oc_enr_pay`
							...
							LEFT JOIN `schoolenrollmentregistration` `schl_enr_reg`
								ON `oc_enr_pay`.`registration_id` = `schl_enr_reg`.`SchlEnrollRegSms_ID`
								
							LEFT JOIN `schoolenrollmentadmission` `schl_enr_adm` -- FOR ADMISSION
								ON (
									IFNULL(`schl_enr_reg`.`SchlEnrollRegSms_ID`, 0) = 0
									AND `schl_enr_reg`.`SchlEnrollReg_ID` = `schl_enr_adm`.`SchlEnrollReg_ID`
									AND IFNULL(`schl_enr_adm`.`SchlEnrollAdmSms_ID`, 0) = 0
								) OR (
									IFNULL(`schl_enr_reg`.`SchlEnrollRegSms_ID`, 0) != 0
									AND `schl_enr_reg`.`SchlEnrollRegSms_ID` = `schl_enr_adm`.`SchlEnrollReg_ID`
								)
							...
							LEFT JOIN `schoolenrollmentassessment` `schl_acad_asmt` -- FOR ASSESSMENT
								ON	(CASE IFNULL(`schl_enr_adm`.`SchlEnrollAdmSms_ID`, 0)
									...
									END) = `schl_acad_asmt`.`SchlEnrollAdm_ID`
							...
							LEFT JOIN `schoolacademicperiod` `schl_acad_prd`
								ON `schl_enr_reg`.`SchlAcadPrd_ID` = `schl_acad_prd`.`SchlAcadPrdSms_ID`

						WHERE 	`schl_enr_reg`.`SchlAcadLvl_ID` = $lvlid AND 
								`schl_enr_reg`.`SchlAcadYr_ID`  = $YR_ID AND 
								`schl_enr_reg`.`SchlAcadPrd_ID` = $PRD_ID 

						GROUP BY (CASE IFNULL(`schl_enr_reg`.`SchlEnrollRegSms_ID`, 0)
			...
			$rsreg = $dbPortal->query($qry);
			$rows = $rsreg->fetch_ALL(MYSQLI_ASSOC);
			$stud_container = array_merge($stud_container, $rows);
		}
